...
export function setupAddFlow({ getShoppingList, setShoppingList, getSelectedStore, saveAndRender }) {
    ...
    // Simple loading UI helper: disables add button and shows a small message
    function setAddLoading(isLoading) {
        try {
            ...
            if (isLoading) {
                let el = suggestionsDiv.querySelector('.wkf-loading-line');
                if (!el) {
                ...
                const el = suggestionsDiv.querySelector('.wkf-loading-line');
                if (el) el.remove();
            }
    ...
    async function addItemFromInputs() {
        // show loading UI immediately so the user gets feedback
        setAddLoading(true);
        try {
            if (!itemInput) return;
    ...
    // Expose helper so other modules can toggle loading if needed
    window.setAddLoading = setAddLoading;
